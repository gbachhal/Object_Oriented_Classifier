...
class ClassifyByTopN(ClassifyByTarget):
    ...
    def target_top_n(self, tset, num=5, label=''):
        all_words = []
        for ti in tset.inObjHash:
            if ti.get_label == label:
                all_words.extend(ti.get_words())
            all_words = sorted(all_words)

        totalwordslist = {}
        for i in range(len(all_words)):
            totalwordslist[all_words[i]] = all_words.count(all_words[i])

        wordlist = list(totalwordslist.keys())

        x = []                               # from Raghav Sharma
        while len(x) < num:                                     #
            max_freq = max(totalwordslist.values())             #
            for i in wordlist:                                  #
                if totalwordslist[i] == max_freq:               #
                    x.append(i)                                 #
                    totalwordslist.pop(i)                       #
                    wordlist.remove(i)                          #
        self.targetWords = sorted(x)                            #
        return self.targetWords
...
class TrainingInstance(C274):
    ...
    def preprocess_words(self, mode=''):
        self.preprocessWord = []                        # from Parshva Shah
        self.processWords(self.inst["words"], mode)     # from Parshva Shah
        self.inst["words"] = self.preprocessWord        # from Parshva Shah

    def processWords(self, line, mode):
        for word in line:
            if (mode == self.modes[0]):
                word = self.removeSymbols(word)
                word = self.removeStopWords(word)
            elif (mode == self.modes[1]):
                word = self.removeDigits(word)
                word = self.removeStopWords(word)
                ...
                word = self.removeStopWords(word)
            if word != '':
                self.preprocessWord.append(word)

    def removeSymbols(self, word):  # removes all symbols
        return word.translate(str.maketrans('', '', string.punctuation))

    def removeDigits(self, word):  # removes all digits
        digitFlag = True
        for i in word:
            if i.isdigit():
                pass
    ...
    def removeStopWords(self, word):  # removes all StopWords
        return "" if word in self.Stop_Words else word
...
class TrainingSet(C274):
    def __init__(self):
        ...
        self.tsNlist = []       # from Parshva Shah
        return
    ...
    def return_nfolds(self, num=3):
        ...
        tsets = []
        for i in range(num):
            new_groups = instances[i::num]
            r_robin.append(new_groups)
        for group in r_robin:                              # from Uzair Tariq
            new_tset = TrainingSet()                                        #
            new_tset.inObjHash = copy.deepcopy(group)                       #
            for ti in group:                                                #
                line = f"{ti.get_label()}\t{' '.join(ti.get_words())}"      #
                new_tset.inObjList.append(line)                             #
        tsets.append(new_tset)                                              #

    def copy(self):
        return copy.deepcopy(self)

    def add_training_set(self, tset):
        for i in range(len(tset.inObjHash)):          # from Parshva Shah
